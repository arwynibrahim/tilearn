...
describe('VrService', ()=>{
    ...
    beforeEach(async ()=>{
        ...
        service = module.get(_vrservice.VrService);
        jest.clearAllMocks();
    });
    describe('createScene', ()=>{
        it('should create a VR scene', async ()=>{
            ...
            mockPrisma.vRScene.create.mockResolvedValue({
                id: 'scene-1',
                ...dto
            });
            ...
            expect(mockPrisma.vRScene.create).toHaveBeenCalledWith({
                data: dto
            });
    ...
    describe('findSceneByModule', ()=>{
        it('should return scene for module', async ()=>{
            mockPrisma.vRScene.findUnique.mockResolvedValue({
                id: 's1',
                moduleId: 'mod-1'
            });
            const result = await service.findSceneByModule('mod-1');
            expect(mockPrisma.vRScene.findUnique).toHaveBeenCalledWith({
                where: {
                    moduleId: 'mod-1'
                }
            });
        ...
        it('should throw NotFoundException if no scene', async ()=>{
            ...
            await expect(service.findSceneByModule('mod-1')).rejects.toThrow(_common.NotFoundException);
        });
    });
    describe('findOneScene', ()=>{
        ...
        it('should throw NotFoundException if not found', async ()=>{
            mockPrisma.vRScene.findUnique.mockResolvedValue(null);
            await expect(service.findOneScene('bad')).rejects.toThrow(_common.NotFoundException);
        });
    });
    describe('updateScene', ()=>{
        it('should update a scene', async ()=>{
            ...
            mockPrisma.vRScene.update.mockResolvedValue({
                id: 's1',
                title: 'Updated'
            });
            const result = await service.updateScene('s1', {
                title: 'Updated'
            });
            expect(mockPrisma.vRScene.findUnique).toHaveBeenCalledWith({
                where: {
            ...
            expect(mockPrisma.vRScene.update).toHaveBeenCalledWith({
                where: {
                    id: 's1'
                },
                data: {
        ...
        it('should throw NotFoundException if scene not found', async ()=>{
            mockPrisma.vRScene.findUnique.mockResolvedValue(null);
            await expect(service.updateScene('bad', {
                title: 'New'
            })).rejects.toThrow(_common.NotFoundException);
        });
    });
...
//# sourceMappingURL=vr.service.spec.js.map