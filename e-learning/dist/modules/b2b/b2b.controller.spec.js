...
describe('B2bController', ()=>{
    ...
    beforeEach(async ()=>{
        const module = await _testing.Test.createTestingModule({
            ...
            providers: [
                {
                    provide: _b2bservice.B2bService,
                    useValue: mockB2bService
                }
            ]
        }).compile();
        controller = module.get(_b2bcontroller.B2bController);
        jest.clearAllMocks();
    });
    it('createOrganization', async ()=>{
        ...
        mockB2bService.createOrganization.mockResolvedValue({
            id: 'o1'
        });
        expect(await controller.createOrganization(dto)).toEqual({
            id: 'o1'
        });
    ...
    it('findOneOrganization', async ()=>{
        mockB2bService.findOneOrganization.mockResolvedValue({
            id: 'o1'
        });
        expect(await controller.findOneOrganization('o1')).toEqual({
            id: 'o1'
        });
    ...
    it('createLicense', async ()=>{
        ...
        mockB2bService.createLicense.mockResolvedValue({
            id: 'l1'
        });
        expect(await controller.createLicense(dto)).toEqual({
            id: 'l1'
        });
        expect(mockB2bService.createLicense).toHaveBeenCalledWith(dto);
    });
    it('assignLicense', async ()=>{
        mockB2bService.assignLicense.mockResolvedValue({
            id: 'a1'
        });
        expect(await controller.assignLicense('lic-1', 'user-1', 'admin-1')).toEqual({
            id: 'a1'
        });
        expect(mockB2bService.assignLicense).toHaveBeenCalledWith('lic-1', 'user-1', 'admin-1');
    });
    it('revokeLicense', async ()=>{
        mockB2bService.revokeLicense.mockResolvedValue({
            id: 'a1',
            revokedAt: new Date()
        });
    ...
    it('createLearningPath', async ()=>{
        ...
        mockB2bService.createLearningPath.mockResolvedValue({
            id: 'lp1'
        });
        expect(await controller.createLearningPath(dto)).toEqual({
            id: 'lp1'
        });
...
//# sourceMappingURL=b2b.controller.spec.js.map