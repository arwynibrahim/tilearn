...
describe('CatalogueController', () => {
  ...
  describe('createCourse', () => {
    it('should call catalogueService.createCourse with dto and userId', async () => {
      const dto = { title: 'Course', domainId: 'd1' };
  ...
  describe('findAllCourses', () => {
    it('should return courses with filters', async () => {
      const expected = { courses: [], total: 0, page: 1, limit: 20, totalPages: 0 };
      ...
      expect(mockCatalogueService.findAllCourses).toHaveBeenCalledWith(1, 20, { domainId: 'd1', level: 'BEGINNER' });
      expect(result).toEqual(expected);
    });

    it('should convert query params and omit undefined filters', async () => {
      await controller.findAllCourses('2' as any, '10' as any, undefined, undefined);
      expect(mockCatalogueService.findAllCourses).toHaveBeenCalledWith(2, 10, { domainId: undefined, level: undefined });
    });
  });

  describe('findOneCourse', () => {
    it('should return a course by slug', async () => {
      const course = { id: 'c1', slug: 'mon-cours' };
  ...
  describe('removeCourse', () => {
    it('should unpublish a course', async () => {
      mockCatalogueService.removeCourse.mockResolvedValue({ message: 'Cours dépublié' });
