...
describe('CatalogueService', () => {
  ...
  describe('createDomain', () => {
    it('should create a domain', async () => {
      const dto = { name: 'Dev', slug: 'dev', icon: 'code', description: 'Dev courses' };
      mockPrisma.domain.findUnique.mockResolvedValue(null);
      mockPrisma.domain.create.mockResolvedValue({ id: 'domain-1', ...dto });

      const result = await service.createDomain(dto);

      expect(mockPrisma.domain.findUnique).toHaveBeenCalledWith({ where: { slug: 'dev' } });
      expect(mockPrisma.domain.create).toHaveBeenCalledWith({ data: dto });
      expect(result).toEqual({ id: 'domain-1', ...dto });
    });

    it('should throw ConflictException if slug exists', async () => {
      mockPrisma.domain.findUnique.mockResolvedValue({ id: 'existing' });

      await expect(service.createDomain({ name: 'Dev', slug: 'dev' })).rejects.toThrow(ConflictException);
    });
  });

  describe('findAllDomains', () => {
    it('should return all domains with course count', async () => {
      const domains = [{ id: 'd1', name: 'Dev', _count: { courses: 5 } }];
  ...
  describe('findOneDomain', () => {
    it('should return a domain with courses', async () => {
      const domain = { id: 'd1', name: 'Dev', courses: [] };
    ...
    it('should throw NotFoundException if domain not found', async () => {
      ...
      await expect(service.findOneDomain('unknown')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('createCourse', () => {
    it('should create a course with auto-generated slug', async () => {
      ...
      const created = { id: 'course-1', ...dto, slug: 'introduction-au-cloud-aws', createdBy: 'user-1', domain: { id: 'domain-1' } };
      mockPrisma.course.create.mockResolvedValue(created);

      const result = await service.createCourse(dto, 'user-1');

      expect(mockPrisma.course.create).toHaveBeenCalledWith({
        data: { ...dto, slug: 'introduction-au-cloud-aws', createdBy: 'user-1' },
        include: { domain: true },
      });
    ...
    it('should handle special characters in slug generation', async () => {
      const dto = { title: 'Éléctronique & Mécanique!', domainId: 'd1' };
      mockPrisma.course.create.mockResolvedValue({ id: 'c1', slug: 'electronique-mecanique', ...dto, createdBy: 'u1', domain: {} });
  ...
  describe('findAllCourses', () => {
    it('should return paginated courses with filters', async () => {
      ...
      expect(result).toEqual({ courses, total: 1, page: 1, limit: 10, totalPages: 1 });
    });
  ...
  describe('findOneCourse', () => {
    it('should return a course by slug', async () => {
      const course = { id: 'c1', slug: 'mon-cours', domain: {}, creator: {}, modules: [], reviews: [] };
    ...
    it('should throw NotFoundException if course not found', async () => {
      ...
      await expect(service.findOneCourse('unknown')).rejects.toThrow(NotFoundException);
    });
  });

  describe('updateCourse', () => {
    it('should update a course', async () => {
      const existing = { id: 'c1', title: 'Old' };
      const dto = { title: 'New Title' };
      const updated = { id: 'c1', title: 'New Title', slug: 'old', domain: { id: 'd1' } };
  ...
  describe('removeCourse', () => {
    it('should unpublish a course', async () => {
      ...
      expect(result).toEqual({ message: 'Cours dépublié' });
    });
  });

  describe('createModule', () => {
    it('should create a module with auto-incremented order', async () => {
      const dto = { title: 'Module 1', type: 'VIDEO' as const, courseId: 'course-1' };
      const course = { id: 'course-1' };
      const created = { id: 'mod-1', ...dto, order: 1, course: { id: 'course-1', title: 'Course' } };
    ...
    it('should use provided order if specified', async () => {
      const dto = { title: 'Mod', type: 'QUIZ' as const, courseId: 'course-1', order: 5 };
  ...
  describe('findModulesByCourse', () => {
    it('should return modules for a course', async () => {
      const modules = [{ id: 'm1', vrScene: null, quizzes: [] }];
