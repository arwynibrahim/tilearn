...
describe('InstructorController', () => {
  ...
  it('getProfile', async () => {
    mockInstructorService.getProfile.mockResolvedValue({ id: 'p1' });
    expect(await controller.getProfile('user-1')).toEqual({ id: 'p1' });
    expect(mockInstructorService.getProfile).toHaveBeenCalledWith('user-1');
  });

  it('updateProfile', async () => {
    const data = { bio: 'New bio' };
    mockInstructorService.updateProfile.mockResolvedValue({ id: 'p1', bio: 'New bio' });
    expect(await controller.updateProfile('user-1', data)).toEqual({ id: 'p1', bio: 'New bio' });
  ...
  it('getInstructorStats', async () => {
    mockInstructorService.getInstructorStats.mockResolvedValue({ totalCourses: 2, totalEnrollments: 10, totalReviews: 5, averageRating: 4.2 });
    ...
    expect(result).toEqual({ totalCourses: 2, totalEnrollments: 10, totalReviews: 5, averageRating: 4.2 });
  });

  it('createReview', async () => {
    const dto = { courseId: 'c1', rating: 5 };
    mockInstructorService.createReview.mockResolvedValue({ id: 'r1' });
    expect(await controller.createReview(dto, 'user-1')).toEqual({ id: 'r1' });
