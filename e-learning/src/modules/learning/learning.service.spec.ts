...
describe('LearningService', () => {
  ...
  describe('enroll', () => {
    it('should enroll user in a course', async () => {
      ...
      mockPrisma.enrollment.create.mockResolvedValue({ id: 'enr-1', userId: 'user-1', courseId: 'course-1', course: { id: 'course-1', title: 'Course' } });

      const result = await service.enroll({ courseId: 'course-1' }, 'user-1');
      ...
      expect(result).toEqual({ id: 'enr-1', userId: 'user-1', courseId: 'course-1', course: { id: 'course-1', title: 'Course' } });
    });

    it('should throw NotFoundException if course not found', async () => {
      mockPrisma.course.findUnique.mockResolvedValue(null);

      await expect(service.enroll({ courseId: 'bad' }, 'user-1')).rejects.toThrow(NotFoundException);
    });

    it('should throw BadRequestException if already enrolled', async () => {
      mockPrisma.course.findUnique.mockResolvedValue({ id: 'course-1' });
      mockPrisma.enrollment.findFirst.mockResolvedValue({ id: 'existing' });

      await expect(service.enroll({ courseId: 'course-1' }, 'user-1')).rejects.toThrow(BadRequestException);
    });
  });

  describe('getUserEnrollments', () => {
    it('should return user enrollments', async () => {
      const enrollments = [{ id: 'e1', course: { id: 'c1', title: 'Course' } }];
      ...
      expect(mockPrisma.enrollment.findMany).toHaveBeenCalledWith({
        where: { userId: 'user-1' },
        include: { course: { select: { id: true, title: true, slug: true, thumbnail: true, level: true } } },
  ...
  describe('updateProgress', () => {
    it('should create progress if not existing', async () => {
      ...
      mockPrisma.progress.create.mockResolvedValue({ id: 'p1', userId: 'u1', moduleId: 'mod-1', status: 'IN_PROGRESS' });

      const result = await service.updateProgress('u1', 'mod-1', { status: 'IN_PROGRESS' });

      expect(mockPrisma.progress.create).toHaveBeenCalledWith({
        data: { userId: 'u1', moduleId: 'mod-1', status: 'IN_PROGRESS', startedAt: expect.any(Date) },
      });
      expect(result).toEqual({ id: 'p1', userId: 'u1', moduleId: 'mod-1', status: 'IN_PROGRESS' });
    });

    it('should update existing progress', async () => {
      mockPrisma.module.findUnique.mockResolvedValue({ id: 'mod-1' });
      mockPrisma.progress.findUnique.mockResolvedValue({ id: 'p1', userId: 'u1', moduleId: 'mod-1' });
      mockPrisma.progress.update.mockResolvedValue({ id: 'p1', status: 'COMPLETED' });

      const result = await service.updateProgress('u1', 'mod-1', { status: 'COMPLETED' });
    ...
    it('should throw NotFoundException if module not found', async () => {
      ...
      await expect(service.updateProgress('u1', 'bad', {})).rejects.toThrow(NotFoundException);
    });
  });

  describe('getUserProgress', () => {
    it('should return progress with stats', async () => {
      const modules = [{ id: 'm1', title: 'M1' }, { id: 'm2', title: 'M2' }];
      const progress = [{ id: 'p1', moduleId: 'm1', status: 'COMPLETED', score: null, timeSpentSeconds: 0 }];
      ...
      expect(result.stats).toEqual({ totalModules: 2, completedModules: 1, completionPercent: 50 });
  ...
  describe('submitQuiz', () => {
    const quiz = {
      id: 'q1', passingScore: 60,
      questions: [
        { id: 'q1', points: 2, correctAnswer: 'A' },
    ...
    it('should submit quiz and calculate score', async () => {
      ...
      expect(mockPrisma.quizAttempt.create).toHaveBeenCalledWith({
        data: {
          ...
          quizId: 'q1',
          score: expectedScore,
          answers: [{ questionId: 'q1', answer: 'A' }, { questionId: 'q2', answer: 'C' }],
          startedAt: undefined,
  ...
  describe('generateCertificate', () => {
    it('should generate certificate for completed course', async () => {
      ...
      (crypto.createHash as jest.Mock).mockReturnValue({ update: jest.fn().mockReturnValue({ digest: jest.fn().mockReturnValue('hash-123') }) });
      mockPrisma.certificate.create.mockResolvedValue({ id: 'cert-1', certificateUid: 'uuid-123', verificationUrl: 'https://tilearning.net/verify/uuid-123', qrCodeHash: 'hash-123', user: {}, course: {} });

      const result = await service.generateCertificate('user-1', 'course-1');

      expect(mockPrisma.enrollment.findFirst).toHaveBeenCalledWith({
        where: { userId: 'user-1', courseId: 'course-1', status: 'COMPLETED' },
      });
      expect(crypto.randomUUID).toHaveBeenCalled();
      expect(result).toEqual({ id: 'cert-1', certificateUid: 'uuid-123', verificationUrl: 'https://tilearning.net/verify/uuid-123', qrCodeHash: 'hash-123', user: {}, course: {} });
    });

    it('should throw BadRequestException if course not completed', async () => {
      mockPrisma.enrollment.findFirst.mockResolvedValue(null);

      await expect(service.generateCertificate('user-1', 'course-1')).rejects.toThrow(BadRequestException);
    });
  });

  describe('getUserCertificates', () => {
    it('should return non-revoked certificates', async () => {
      mockPrisma.certificate.findMany.mockResolvedValue([{ id: 'c1' }]);
  ...
  describe('verifyCertificate', () => {
    it('should return valid certificate', async () => {
      const cert = { id: 'c1', certificateUid: 'uid', revokedAt: null, user: {}, course: {} };
      ...
      expect(mockPrisma.certificate.findUnique).toHaveBeenCalledWith({
        ...
        include: { user: { select: { nom: true, prenom: true } }, course: { select: { title: true, domain: true } } },
      });
      expect(result).toEqual({ valid: true, ...cert });
    });

    it('should throw NotFoundException if revoked', async () => {
      mockPrisma.certificate.findUnique.mockResolvedValue({ id: 'c1', revokedAt: new Date() });

      await expect(service.verifyCertificate('uid')).rejects.toThrow(NotFoundException);
    });

    it('should throw NotFoundException if not found', async () => {
      mockPrisma.certificate.findUnique.mockResolvedValue(null);

      await expect(service.verifyCertificate('unknown')).rejects.toThrow(NotFoundException);
    });
  });
});
