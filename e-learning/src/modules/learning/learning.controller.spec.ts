...
describe('LearningController', () => {
  ...
  describe('enroll', () => {
    it('should call service.enroll', async () => {
      mockLearningService.enroll.mockResolvedValue({ id: 'e1' });
      const result = await controller.enroll({ courseId: 'c1' }, 'user-1');
      expect(mockLearningService.enroll).toHaveBeenCalledWith({ courseId: 'c1' }, 'user-1');
  ...
  describe('updateProgress', () => {
    it('should call service.updateProgress', async () => {
      ...
      mockLearningService.updateProgress.mockResolvedValue({ id: 'p1' });
  ...
  describe('getUserProgress', () => {
    it('should call service.getUserProgress', async () => {
      mockLearningService.getUserProgress.mockResolvedValue({ modules: [], stats: {} });
      ...
      expect(result).toEqual({ modules: [], stats: {} });
    });
  });

  describe('submitQuiz', () => {
    it('should call service.submitQuiz', async () => {
      const dto = { quizId: 'q1', answers: [] };
      mockLearningService.submitQuiz.mockResolvedValue({ attempt: {}, score: 100, passed: true, totalQuestions: 1, correctAnswers: 1 });
      const result = await controller.submitQuiz(dto, 'user-1');
      expect(mockLearningService.submitQuiz).toHaveBeenCalledWith(dto, 'user-1');
      expect(result).toEqual({ attempt: {}, score: 100, passed: true, totalQuestions: 1, correctAnswers: 1 });
    });
  });
  ...
  describe('generateCertificate', () => {
    it('should call service.generateCertificate', async () => {
      mockLearningService.generateCertificate.mockResolvedValue({ id: 'cert-1' });
  ...
  describe('verifyCertificate', () => {
    it('should call service.verifyCertificate', async () => {
      mockLearningService.verifyCertificate.mockResolvedValue({ valid: true, id: 'c1' });
      ...
      expect(result).toEqual({ valid: true, id: 'c1' });
    });
  });
});
